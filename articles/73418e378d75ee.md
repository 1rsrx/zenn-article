...
## まとめ
- Delegateメソッドの呼び出しは、処理を移譲する側(UITableViewとか)が決める
- Delegateメソッドの処理内容は、処理を移譲される側(UIViewControllerとか)が決める
- 呼び出しは、移譲する側のdelegateプロパティを介して行われる(`delegate?.tableView(self, didSelectRowAt: indexPath)`)
- そのため、delegateプロパティに移譲されるクラスをセットする(`tableView.delegate = self`)
